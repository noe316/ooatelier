...
document.addEventListener('scroll', function(){
  ...
  // var pieTop = document.getElementsByClassName('sec6')
  var pieStart = $('.sec6').offset().top
  var charts = [];
  ...
  if(wTop < pieStart) {
    if(winW > 768) {
      [].forEach.call(document.querySelectorAll('.chart_bw'),  function(el) {
        charts.push(new EasyPieChart(el, optionsBw_w));
      });
    
      [].forEach.call(document.querySelectorAll('.chart_color'),  function(el) {
       charts.push(new EasyPieChart(el, optionsColor_w));
     });
    } else if(winW <= 768) {
      [].forEach.call(document.querySelectorAll('.chart_bw'),  function(el) {
        charts.push(new EasyPieChart(el, optionsBw_t));
      });
    
      [].forEach.call(document.querySelectorAll('.chart_color'),  function(el) {
       charts.push(new EasyPieChart(el, optionsColor_t));
     });
    }
...
//   var wTop = $(window).scrollTop()
//   var pieStart = $('.fact2').offset().wTop
//   var charts = [];

//   if(wTop > pieStart)  {
//     [].forEach.call(document.querySelectorAll('.chart_bw'),  function(el) {
//       charts.push(new EasyPieChart(el, optionsBw));
//     });
  
//     [].forEach.call(document.querySelectorAll('.chart_color'),  function(el) {
//      charts.push(new EasyPieChart(el, optionsColor));
//    });
//   }
